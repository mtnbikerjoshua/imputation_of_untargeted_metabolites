...
#'
#' Equivalent to \code{mice::as.mids} except with the ability to input a predictor
#' matrix. This is helpful for very large datasets where the original
#' imputation was done using only the most correlated predictors.
#' 
...
#' @param predictorMatrix Passed on to \code{mice::mice}
#' @inheritParams mice
...
#' @keywords mids
#' @export
as.mids <- function(long, where = NULL, .imp = ".imp", .id = ".id", predictorMatrix) {
  require(mice)
